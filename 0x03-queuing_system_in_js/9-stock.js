...
const getItemById = (id) => {
    const item = listProducts.find(obj.itemId === id);
 ...
 * @param {number} itemId - id of a given item
...
const getReservedStockById = async (itemId) => {
    return promisify(client.GET).bind(client)(`item.${itemId}`);
};

app.get('/list_products', (req, res) => {
    reserveStockById.json(listProducts);
});

app.get('/list_products/:itemId(\\d+)', (_, res) => {
